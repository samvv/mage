...
def for_each_py_arg(node: PyArg, proc: Callable[[PyArg], None]):
    ...
    if isinstance(node, PyGeneratorExpr):
        proc(node.element)
        for element_1 in node.generators:
            proc(element_1.target)
            for element_2 in element_1.guards:
                proc(element_2.expr)
        return
    ...
    if isinstance(node, PySubscriptExpr):
        proc(node.expr)
        for (element_4, separator_2) in node.slices:
            if is_py_expr(element_4):
                proc(element_4)
            elif isinstance(element_4, PyExprSlice):
                if is_py_expr(element_4.lower):
                    proc(element_4.lower)
                if is_py_expr(element_4.upper):
                    proc(element_4.upper)
                if isinstance(element_4.step, tuple):
                    proc(element_4.step[1])
        return
    if isinstance(node, PyTupleExpr):
        for (element_5, separator_3) in node.elements:
            proc(element_5)
        return
...
def for_each_py_expr(node: PyExpr, proc: Callable[[PyExpr], None]):
    ...
    if isinstance(node, PyCallExpr):
        proc(node.operator)
        for (element, separator) in node.args:
            if isinstance(element, PyKeywordArg):
                proc(element.expr)
...
def for_each_py_pattern(node: PyPattern, proc: Callable[[PyPattern], None]):
    ...
    if isinstance(node, PySubscriptPattern):
        proc(node.pattern)
        for (element_1, separator_1) in node.slices:
            if is_py_pattern(element_1):
                proc(element_1)
            elif isinstance(element_1, PyPatternSlice):
                if is_py_pattern(element_1.lower):
                    proc(element_1.lower)
                if is_py_pattern(element_1.upper):
                    proc(element_1.upper)
                if isinstance(element_1.step, tuple):
                    proc(element_1.step[1])
        return
...
def for_each_py_stmt(node: PyStmt, proc: Callable[[PyStmt], None]):
    ...
    if isinstance(node, PyIfStmt):
        if is_py_stmt(node.first.body):
            ...
            for element_4 in node.first.body:
                proc(element_4)
        for element_5 in node.alternatives:
            if is_py_stmt(element_5.body):
                proc(element_5.body)
            elif isinstance(element_5.body, list):
                for element_6 in element_5.body:
                    proc(element_6)
        if isinstance(node.last, PyElseCase):
            if is_py_stmt(node.last.body):
                proc(node.last.body)
            elif isinstance(node.last.body, list):
                for element_7 in node.last.body:
                    proc(element_7)
        return
    ...
    if isinstance(node, PyWhileStmt):
        ...
        if isinstance(node.else_clause, tuple):
            if is_py_stmt(node.else_clause[2]):
                ...
                for element_14 in node.else_clause[2]:
                    proc(element_14)
        return
...
def rewrite_each_py_arg(node: PyArg, proc: Callable[[PyArg], PyArg]) -> PyArg:
    if isinstance(node, PyAttrExpr):
        ...
        if new_expr is not node.expr:
            changed = True
        if changed:
            return PyAttrExpr(expr=new_expr, dot=node.dot, name=node.name)
        else:
            return node
    if isinstance(node, PyCallExpr):
        changed = False
        ...
        if new_operator is not node.operator:
            changed = True
        new_args = Punctuated()
        for (element, separator) in node.args.delimited:
            new_element = proc(element)
            assert(is_py_arg(new_element))
            if new_element is not element:
                changed = True
            new_separator = separator
            new_args.append(new_element, new_separator)
        new_last = proc(node.args.last)
        assert(is_py_arg(new_last))
        if new_last is not node.args.last:
            changed = True
        new_args.append_final(new_last)
        if changed:
    ...
    if isinstance(node, PyGeneratorExpr):
        changed = False
        new_element_1 = proc(node.element)
        assert(is_py_expr(new_element_1))
        if new_element_1 is not node.element:
            changed = True
        if changed:
            return PyGeneratorExpr(element=new_element_1, generators=node.generators)
        else:
            return node
    if isinstance(node, PyIfExpr):
        changed = False
    ...
    if isinstance(node, PyKeywordArg):
        ...
        if new_expr_1 is not node.expr:
            changed = True
        if changed:
            return PyKeywordArg(name=node.name, equals=node.equals, expr=new_expr_1)
        else:
            return node
    if isinstance(node, PyListExpr):
        changed = False
        ...
        for (element_1, separator_1) in node.elements.delimited:
            ...
            new_elements.append(new_element_2, new_separator_1)
        ...
        if changed:
            return PyListExpr(open_bracket=node.open_bracket, elements=new_elements, close_bracket=node.close_bracket)
        else:
            return node
    if isinstance(node, PyNamedExpr):
        return node
    ...
    if isinstance(node, PyPrefixExpr):
        ...
        if new_expr_3 is not node.expr:
            changed = True
        if changed:
            return PyPrefixExpr(prefix_op=node.prefix_op, expr=new_expr_3)
        else:
            return node
    if isinstance(node, PyStarredExpr):
        changed = False
        ...
        if new_expr_4 is not node.expr:
            changed = True
        if changed:
            return PyStarredExpr(asterisk=node.asterisk, expr=new_expr_4)
        else:
            return node
    if isinstance(node, PySubscriptExpr):
        changed = False
        ...
        for (element_2, separator_2) in node.slices.delimited:
            if is_py_expr(element_2):
                ...
                if is_py_expr(element_2.lower):
                    new_lower = proc(element_2.lower)
                    assert(is_py_expr(new_lower))
                    if new_lower is not element_2.lower:
                        changed = True
                elif element_2.lower is None:
                    new_lower = element_2.lower
                else:
                    assert_never(element_2.lower)
                if is_py_expr(element_2.upper):
                    new_upper = proc(element_2.upper)
                    assert(is_py_expr(new_upper))
                    if new_upper is not element_2.upper:
                        changed = True
                elif element_2.upper is None:
                    new_upper = element_2.upper
                else:
                    assert_never(element_2.upper)
                if isinstance(element_2.step, tuple):
                    new_elements_1 = []
                    element_0 = element_2.step[0]
                    new_element_0 = element_0
                    new_elements_1.append(new_element_0)
                    element_1 = element_2.step[1]
                    new_element_1 = proc(element_1)
                    assert(is_py_expr(new_element_1))
                    if new_element_1 is not element_1:
                        changed = True
                    new_elements_1.append(new_element_1)
                    new_step = tuple(new_elements_1) if changed else element_2.step
                elif element_2.step is None:
                    new_step = element_2.step
                else:
                    assert_never(element_2.step)
                if changed:
                    new_element_3 = PyExprSlice(lower=new_lower, colon=element_2.colon, upper=new_upper, step=new_step)
                else:
                    new_element_3 = element_2
            else:
                assert_never(element_2)
            new_separator_2 = separator_2
            new_slices.append(new_element_3, new_separator_2)
        if is_py_expr(node.slices.last):
            new_last_2 = proc(node.slices.last)
            assert(is_py_expr(new_last_2))
            if new_last_2 is not node.slices.last:
                changed = True
            ...
            if isinstance(node.slices.last.step, tuple):
                ...
                new_elements_2.append(new_element_0_1)
                ...
                if new_element_1_1 is not element_1_1:
                    changed = True
                new_elements_2.append(new_element_1_1)
                new_step_1 = tuple(new_elements_2) if changed else node.slices.last.step
            elif node.slices.last.step is None:
                new_step_1 = node.slices.last.step
            else:
                assert_never(node.slices.last.step)
            if changed:
                new_last_2 = PyExprSlice(lower=new_lower_1, colon=node.slices.last.colon, upper=new_upper_1, step=new_step_1)
            else:
                new_last_2 = node.slices.last
        else:
            assert_never(node.slices.last)
        new_slices.append_final(new_last_2)
        if changed:
            return PySubscriptExpr(expr=new_expr_5, open_bracket=node.open_bracket, slices=new_slices, close_bracket=node.close_bracket)
        else:
            return node
    if isinstance(node, PyTupleExpr):
        changed = False
        new_elements_3 = Punctuated()
        for (element_3, separator_3) in node.elements.delimited:
            new_element_4 = proc(element_3)
            assert(is_py_expr(new_element_4))
            if new_element_4 is not element_3:
                changed = True
            new_separator_3 = separator_3
            new_elements_3.append(new_element_4, new_separator_3)
        new_last_3 = proc(node.elements.last)
        assert(is_py_expr(new_last_3))
        if new_last_3 is not node.elements.last:
            changed = True
        new_elements_3.append_final(new_last_3)
        if changed:
...
def rewrite_each_py_expr(node: PyExpr, proc: Callable[[PyExpr], PyExpr]) -> PyExpr:
    ...
    if isinstance(node, PyCallExpr):
        ...
        for (element, separator) in node.args.delimited:
            if isinstance(element, PyKeywordArg):
                changed = False
                new_expr_1 = proc(element.expr)
                assert(is_py_expr(new_expr_1))
                if new_expr_1 is not element.expr:
                    changed = True
                if changed:
                    new_element = PyKeywordArg(name=element.name, equals=element.equals, expr=new_expr_1)
                else:
                    new_element = element
            elif is_py_expr(element):
                new_element = proc(element)
        ...
        if isinstance(node.args.last, PyKeywordArg):
            changed = False
            new_expr_2 = proc(node.args.last.expr)
            assert(is_py_expr(new_expr_2))
            if new_expr_2 is not node.args.last.expr:
                changed = True
            if changed:
                new_last = PyKeywordArg(name=node.args.last.name, equals=node.args.last.equals, expr=new_expr_2)
            else:
                new_last = node.args.last
    ...
    if isinstance(node, PyNestExpr):
        ...
        if new_expr_3 is not node.expr:
            changed = True
        if changed:
            return PyNestExpr(open_paren=node.open_paren, expr=new_expr_3, close_paren=node.close_paren)
        else:
            return node
    if isinstance(node, PyPrefixExpr):
        changed = False
    ...
    if isinstance(node, PySubscriptExpr):
        ...
        new_slices.append_final(new_last_2)
        if changed:
            return PySubscriptExpr(expr=new_expr_6, open_bracket=node.open_bracket, slices=new_slices, close_bracket=node.close_bracket)
        else:
            return node
    if isinstance(node, PyTupleExpr):
        changed = False
...
def rewrite_each_py_pattern(node: PyPattern, proc: Callable[[PyPattern], PyPattern]) -> PyPattern:
    ...
    if isinstance(node, PyListPattern):
        changed = False
        new_elements = Punctuated()
        for (element, separator) in node.elements.delimited:
            new_element = proc(element)
            assert(is_py_pattern(new_element))
            if new_element is not element:
                changed = True
            new_separator = separator
            new_elements.append(new_element, new_separator)
        new_last = proc(node.elements.last)
        assert(is_py_pattern(new_last))
        if new_last is not node.elements.last:
            changed = True
        new_elements.append_final(new_last)
        if changed:
    ...
    if isinstance(node, PyStarredPattern):
        ...
        if new_pattern_1 is not node.pattern:
            changed = True
        if changed:
            return PyStarredPattern(asterisk=node.asterisk, pattern=new_pattern_1)
        else:
            return node
    if isinstance(node, PySubscriptPattern):
        changed = False
        ...
        if new_pattern_2 is not node.pattern:
            changed = True
        new_slices = Punctuated()
        for (element_1, separator_1) in node.slices.delimited:
            if is_py_pattern(element_1):
                new_element_1 = proc(element_1)
                ...
                changed = False
                if is_py_pattern(element_1.lower):
                    new_lower = proc(element_1.lower)
                    assert(is_py_pattern(new_lower))
                    if new_lower is not element_1.lower:
                        changed = True
                elif element_1.lower is None:
                    new_lower = element_1.lower
                else:
                    assert_never(element_1.lower)
                if is_py_pattern(element_1.upper):
                    new_upper = proc(element_1.upper)
                    assert(is_py_pattern(new_upper))
                    if new_upper is not element_1.upper:
                        changed = True
                elif element_1.upper is None:
                    new_upper = element_1.upper
                else:
                    assert_never(element_1.upper)
                if isinstance(element_1.step, tuple):
                    new_elements_1 = []
                    element_0 = element_1.step[0]
                    new_element_0 = element_0
                    new_elements_1.append(new_element_0)
                    element_1 = element_1.step[1]
                    new_element_1 = proc(element_1)
                    assert(is_py_pattern(new_element_1))
                    if new_element_1 is not element_1:
                        changed = True
                    new_elements_1.append(new_element_1)
                    new_step = tuple(new_elements_1) if changed else element_1.step
                elif element_1.step is None:
                    new_step = element_1.step
                else:
                    assert_never(element_1.step)
                if changed:
                    new_element_1 = PyPatternSlice(lower=new_lower, colon=element_1.colon, upper=new_upper, step=new_step)
                else:
                    new_element_1 = element_1
            else:
                assert_never(element_1)
            new_separator_1 = separator_1
            new_slices.append(new_element_1, new_separator_1)
        if is_py_pattern(node.slices.last):
            new_last_1 = proc(node.slices.last)
            assert(is_py_pattern(new_last_1))
            if new_last_1 is not node.slices.last:
                changed = True
            ...
            if isinstance(node.slices.last.step, tuple):
                ...
                assert_never(node.slices.last.step)
            if changed:
                new_last_1 = PyPatternSlice(lower=new_lower_1, colon=node.slices.last.colon, upper=new_upper_1, step=new_step_1)
            else:
                new_last_1 = node.slices.last
        else:
            assert_never(node.slices.last)
        new_slices.append_final(new_last_1)
        if changed:
            return PySubscriptPattern(pattern=new_pattern_2, open_bracket=node.open_bracket, slices=new_slices, close_bracket=node.close_bracket)
        else:
            return node
    if isinstance(node, PyTuplePattern):
        changed = False
...
def rewrite_each_py_stmt(node: PyStmt, proc: Callable[[PyStmt], PyStmt]) -> PyStmt:
    ...
    if isinstance(node, PyClassDef):
        ...
        if changed:
            return PyClassDef(decorators=node.decorators, class_keyword=node.class_keyword, name=node.name, bases=node.bases, colon=node.colon, body=new_body)
        else:
            return node
    if isinstance(node, PyContinueStmt):
        return node
    ...
    if isinstance(node, PyForStmt):
        ...
        if changed:
            return PyForStmt(for_keyword=node.for_keyword, pattern=node.pattern, in_keyword=node.in_keyword, expr=node.expr, colon=node.colon, body=new_body_1, else_clause=new_else_clause)
        else:
            return node
    if isinstance(node, PyFuncDef):
        changed = False
        if is_py_stmt(node.body):
            ...
            assert_never(node.body)
        if changed:
            return PyFuncDef(decorators=node.decorators, async_keyword=node.async_keyword, def_keyword=node.def_keyword, name=node.name, open_paren=node.open_paren, params=node.params, close_paren=node.close_paren, return_type=node.return_type, colon=node.colon, body=new_body_2)
        else:
            return node
    if isinstance(node, PyGlobalStmt):
        return node
    ...
    if isinstance(node, PyTryStmt):
        ...
        if isinstance(node.finally_clause, tuple):
            ...
            new_finally_clause = tuple(new_elements_2) if changed else node.finally_clause
        elif node.finally_clause is None:
            new_finally_clause = node.finally_clause
        else:
            assert_never(node.finally_clause)
        if changed:
            return PyTryStmt(try_keyword=node.try_keyword, colon=node.colon, body=new_body_3, handlers=node.handlers, else_clause=new_else_clause_1, finally_clause=new_finally_clause)
        else:
            return node
    if isinstance(node, PyTypeAliasStmt):
        return node
    if isinstance(node, PyWhileStmt):
        ...
        if isinstance(node.else_clause, tuple):
            ...
            new_else_clause_2 = tuple(new_elements_3) if changed else node.else_clause
        elif node.else_clause is None:
            new_else_clause_2 = node.else_clause
        else:
            assert_never(node.else_clause)
        if changed:
            return PyWhileStmt(while_keyword=node.while_keyword, expr=node.expr, colon=node.colon, body=new_body_4, else_clause=new_else_clause_2)
        else:
            return node
